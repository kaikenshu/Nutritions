...
past_data = list(nt.find({"User": username}).sort("Date", -1).limit(30))
prd = pr.find()
...
#rolling averages
ntt = list(nt.find({"$and":[{"User":username},{"Date":{"$gt":str(datetime.datetime.strptime(today_date,"%Y-%m-%d")-datetime.timedelta(days=31))}},{"Date":{"$lt":str(datetime.datetime.strptime(today_date,"%Y-%m-%d"))}}]}))
...
if st.button(label="Add"):
    if dropdown == " " or quantity==None:
        ...
        if date in df['Date'].values:
            todaydata=nt.find_one({"$and":[{"User":username},{"Date":date}]})
            fooditem=ni.find_one({"$and":[{"User":username},{"Food":dropdown}]})
...
if st.button(label="Update"):
    ...
    editedfood["User"] = username
    ni.replace_one({"$and":[{"User":username},{"Food":dropdown}]},editedfood)
    st.write("Done!")
...
if st.session_state.get("cook_pressed", False):
    if st.button(label="Confirm"):
        try:
            ...
            if "authentication_status" in st.session_state and username:
                aggregated_data_dict["User"] = username
    ...
    if input_data:
        try:
            ...
            food_data = {
                ...
                "Protein": protein,
                "Fiber": fiber,
                "User": username  # Add the current user
            }
